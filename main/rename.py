...
async def rename_file(bot, msg):
    ...
    sts = await msg.reply_text("<code> ✨ Fetching... </code>")
    c_time = time.time()
    ...
    c_time = time.time()
    try:
        await bot.send_document(msg.chat.id, document=downloaded, thumb=og_thumbnail, caption=cap, progress=progress_message, progress_args=("<b>🔖UPLOADING TO DB..</b>", sts, c_time))        
    except Exception as e:  
